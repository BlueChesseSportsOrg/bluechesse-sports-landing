...
const useCart = (): [CartState, Checkout] => {
  ...
  const addItem = useCallback(
    async (variantId: string, quantity: number) => {
      ...
      const newCart: any = await client.checkout.addLineItems(
        checkoutId,
        lineItemsToAdd
      );
      setCart(newCart);
  ...
  const updateQuantity = useCallback(
    async (lineItemId: string, quantity: number) => {
      ...
      const newCart: any = await client.checkout.updateLineItems(
        checkoutId,
        lineItemsToUpdate
      );
      setCart(newCart);
  ...
  const removeItem = useCallback(
    async (lineItemId: string) => {
      ...
      const newCart: any = await client.checkout.removeLineItems(
